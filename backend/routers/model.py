...
class Model:
    ...
    def extend_dataframe(self, products_df):
        ...
        organizations.rename(columns={"id": "organization_id", "name": "organization_name"}, inplace=True)
        ...
        df_temp = pd.merge(df_temp, organizations[["organization_id", "municipality", "organization_name"]], on="organization_id", how="inner")

        df_temp["name"] = df_temp["name"].str.strip()

        return df_temp

    def user_info(self, path, user_id):
        products_df = self.extend_dataframe(pd.read_csv(path))
    ...
    def get_answer(self, path, query, user_id):
        products_df = pd.read_csv(path)

        vector_db = vector_database.VectorDB()
        vector_db.load_vectordb(path)
    ...
    def get_full_answer(self, path, query, user_id):
        ...
        for product in result:
            try:
                ...
                final_result.append(f"{product} {temp_df.iloc[0, 3]} {temp_df.iloc[0, -2]} {temp_df.iloc[0, -1]}")
            except:
                pass
